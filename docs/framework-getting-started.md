...
description: Build your first MCP server in under 5 minutes using @mctx-ai/mcp-server. No protocol knowledge needed.
---
...
- **`build` script** -- produces a single bundled JavaScript file. Everything, including all dependencies like `@mctx-ai/mcp-server`, must be bundled into one file. esbuild handles this automatically and processes TypeScript natively without needing a separate compilation step.
...
- **[Tools, Resources, and Prompts](/building-mcp-servers/tools-and-resources)** -- learn the three building blocks that make MCP servers powerful, with real-world examples
- **[Server Requirements](/building-mcp-servers/server-requirements)** -- the complete checklist of what mctx expects from your project
